...
export async function activate(context: vscode.ExtensionContext): Promise<VaporviewApi> {
  ...
  try {
    ...
    console.error('[WF:activate] WASM load/compile FAILED:', err instanceof Error ? err.message : err);
    throw err;
  }
...
function findOpenTabForUri(uri: vscode.Uri): { group: vscode.TabGroup; tab: vscode.Tab } | undefined {
  ...
  for (const group of groups) {
    for (const tab of group.tabs) {
      ...
      } else if (input && typeof input === 'object' && 'uri' in input) {
        const inputUri = (input as { uri?: vscode.Uri }).uri;
        if (inputUri && sameResource(inputUri, uri)) {
          return { group, tab };
        }
      }
    }
